...
module.exports = function parse(modules, opts) {
    ...
    function walk(node) {
        ...
        if (isreqv && node.parent.type === 'VariableDeclarator' && node.parent.id.type === 'Identifier') {
            vars[node.parent.id.name] = varModules[reqid];
        } else if (isreqv && node.parent.type === 'AssignmentExpression' && node.parent.left.type === 'Identifier') {
            vars[node.parent.left.name] = varModules[reqid];
        } else if (isreqv && node.parent.type === 'MemberExpression' && isStaticProperty(node.parent.property) && node.parent.parent.type === 'VariableDeclarator' && node.parent.parent.id.type === 'Identifier') {
            var v = varModules[reqid][resolveProperty(node.parent.property)];
            vars[node.parent.parent.id.name] = v;
        } else if (isreqv && node.parent.type === 'MemberExpression' && node.parent.property.type === 'Identifier') {
            //vars[node.parent.parent.id.name] = varModules[reqid];
        } else if (isreqv && node.parent.type === 'CallExpression') {
            //
        }

        if (isreqm && node.parent.type === 'VariableDeclarator' && node.parent.id.type === 'Identifier') {
            ...
            if (decs.length) {
                ...
                decs.forEach(function (d, i) {
                    var key = d.start + skipOffset + ',' + (d.end + skipOffset);
                    skip[key] = true;
        ...
        } else if (isreqm && node.parent.type === 'AssignmentExpression' && node.parent.left.type === 'Identifier') {
        ...
        } else if (isreqm && node.parent.type === 'MemberExpression' && isStaticProperty(node.parent.property) && node.parent.parent.type === 'VariableDeclarator' && node.parent.parent.id.type === 'Identifier') {
            varNames[node.parent.parent.id.name] = [reqid, resolveProperty(node.parent.property)];
        ...
        } else if (isreqm && node.parent.type === 'MemberExpression' && isStaticProperty(node.parent.property)) {
    ...
    function traverse(node, val) {
        for (var p = node; p; p = p.parent) {
            ...
            var key = p.start + skipOffset + ',' + (p.end + skipOffset);
            if (skip[key]) {
                skip[key] = false;
                return;
            }
        ...
        if (node.parent.type === 'CallExpression') {
            if (typeof val !== 'function') {
                return error('tried to statically call ' + inspect(val) + ' as a function');
            }
            ...
            if (!isStaticProperty(node.parent.property)) {
                return error('dynamic property in member expression: ' + node.parent.source());
            }
            ...
            if (cur.type === 'MemberExpression') {
                cur = cur.parent;
                if (cur.type !== 'CallExpression' && cur.parent.type === 'CallExpression') {
                    cur = cur.parent;
                }
            }
            if (node.parent.type === 'MemberExpression' && cur.type !== 'CallExpression' && cur.type !== 'MemberExpression') {
                cur = node.parent;
            }
            ...
            output.emit('error', new Error('unsupported type for static module: ' + node.parent.type + '\nat expression:\n\n  ' + unparse(node.parent) + '\n'));
        }
    }
};
...
function isStream(s) {
    return s && (typeof s === 'undefined' ? 'undefined' : _typeof(s)) === 'object' && typeof s.pipe === 'function';
}

function wrapStream(s) {
    if (typeof s.read === 'function') return s;else return new Readable().wrap(s);
}
