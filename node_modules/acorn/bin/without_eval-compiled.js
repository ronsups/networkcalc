...
walk.simple(ast, {
  ...
  ObjectExpression: function ObjectExpression(node) {
    node.properties.forEach(function (prop) {
      if (prop.value.type == "CallExpression" && prop.value.callee.name == "makePredicate") uses.push(prop.value);
    });
  }
});

var results = [];
var dryRun = acornSrc.slice(0, makePred.end) + "; makePredicate = (function(mp) {" + "return function(words) { var r = mp(words); predicates.push(r); return r }})(makePredicate);" + acornSrc.slice(makePred.end);new Function("predicates", dryRun)(results);
...
//# sourceMappingURL=without_eval-compiled.js.map